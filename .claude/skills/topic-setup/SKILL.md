...
- Desired `engine`: `claude` or `codex`. Prefer Claude Code when both are
  installed. Claude Code CLI is always required by the startup preflight;
  Codex is an optional per-topic engine.
- Desired `exec_mode`: `streaming` (default, headless agent process with live
  progress), `subprocess` for short one-off tasks, or `tmux` for persistent
  terminal coding sessions.
- Desired `stream_mode`: usually `live+` (default); alternatives are `live`,
  `verbose`, and `minimal`.
...
`topic_config.json` with generic defaults (`exec_mode=streaming`,
`stream_mode=live+`). The default runtime engine is `claude`; switch a topic
to `engine=codex` when Codex CLI is installed. Claude Code CLI itself must be
present in any case — the startup preflight requires it.
