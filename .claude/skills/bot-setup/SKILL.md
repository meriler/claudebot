...
- Claude Code CLI **2.1.130 or newer**, installed, authenticated, and
  onboarded for the same user that runs the bot (run `claude` interactively
  once and exit with `/exit`). Claude Code is always required: the startup
  preflight checks for it even when every topic uses Codex.
- Optional: Codex CLI for Codex-powered topics.
...
- `VAULT`: optional path to a workspace/vault directory for file-saving agent
  skills. Leave empty if the user does not use one.
...
`mcp_config=null`, `exec_mode=streaming`, `stream_mode=live+`, and no explicit
engine. Runtime prefers `claude`; a topic can be switched to `engine=codex`
when Codex CLI is installed. Claude Code CLI itself is always required — the
startup preflight fails without it, even in Codex-only setups.
...
- `exec_mode`: `streaming` (default) for a long-lived headless agent process
  with live progress and follow-up injection, `subprocess` for one-off
  assistant tasks where the user does not need a persistent agent process, or
  `tmux` for full development sessions with persistent context, TUI snapshots,
  `/resume`, and `/tui`.
...
- `/clear`: reset the current topic session. In tmux mode, clears logical
  context while keeping the tmux session alive. `/new` still works as a legacy
  alias, but `/clear` is the command shown in the menu.
- `/cancel`: cancel current processing. Use before changing mode if work is in
  progress.
- `/language`: show or switch the bot UI language, for example `/language ru`.
- `/mode`: choose execution transport. `streaming` (default) keeps a headless
  agent process with live progress and follow-up injection; `subprocess` is
  best for one-off tasks; `tmux` is best for full development work, persistent
  context, TUI snapshots, `/resume`, and `/tui`.
- `/engine`: choose Claude Code or Codex for the current forum topic. It resets
  the active session when switching.
- `/stream`: choose progress delivery. `live+` (default) streams intermediate
  agent replies as separate messages, `live` edits one progress buffer,
  `verbose` sends many event messages, `minimal` focuses on final answers.
- `/model`: pick the provider model for the current topic, or reset to the
  engine default. (Not shown in the Telegram command menu, but available.)
- `/sysprompt`: set, show, or reset a custom system prompt for the current
  topic or DM. It is appended on top of the base persona.
- `/health`: run read-only self-checks (CLI presence and onboarding, writable
  paths, config) and report results. The first thing to try when the bot
  misbehaves.
...
When unsure: keep the defaults (`/mode` → streaming, `/stream` → live+), use
`/mode` → tmux for real terminal-driven coding tasks, and `/kill` when an old
tmux session is no longer needed.
...
- Russian/English text did not change: set `BOT_LANG=en` or `BOT_LANG=ru` and
  restart the process.
- Claude Code not found or preflight fails: install Claude Code CLI 2.1.130+
  for the same user that runs the bot, complete onboarding (`claude` once
  interactively, `/exit`), use a PATH where systemd can find it, then restart.
  Codex is optional; if both are installed, topics prefer Claude Code unless
  configured otherwise. `/health` in Telegram shows the same checks.
- Voice messages do not transcribe: create a Deepgram API key, set
  `DEEPGRAM_API_KEY` in `.env`, install `ffmpeg`, and restart.
