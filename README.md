...
Claudebot is an open-source Telegram bot for controlling Claude Code and
...
- `exec_mode`: `streaming` (default), `tmux`, or `subprocess`;
...
The setup this project is built around and the one we recommend:

- a small VPS with Claude Code CLI and this bot running side by side;
- a working directory (for example an Obsidian vault) on the VPS where the
  agent keeps notes, research results, and generated files — point the
  optional `VAULT` env variable at it;
- [Syncthing](https://syncthing.net/) syncing that directory with your local
  computer, so everything the agent writes on the VPS appears in your local
  Obsidian (or any editor) within seconds;
- Telegram as the everyday interface: one forum group, one topic per project
  or long-running context;
- the default modes: `streaming` execution with `live+` stream mode.

None of this is mandatory — the bot works with just a VPS and a bot token —
but the vault-plus-Syncthing loop is what turns it from "remote terminal" into
a persistent assistant whose artifacts live on all your machines.

## Requirements

You need a Linux machine or VPS where the bot and agent CLIs will run. (macOS
also works — see [docs/macos-setup.md](docs/macos-setup.md) for a
LaunchAgent-based guide.)
...
- Claude Code CLI **2.1.130 or newer**, installed and authenticated (Claude
  subscription or API key) for the same Linux user that runs the bot. Run
  `claude` interactively once and exit with `/exit` — the startup preflight
  requires completed onboarding.
- Optional: Codex CLI for Codex-powered topics. Note that Claude Code is still
  required: the startup preflight always checks for it, even if all your topics
  use Codex.
- `tmux` for persistent development sessions
- Optional: Deepgram API key for voice transcription, plus `ffmpeg`
  (strongly recommended for voice — without it transcripts may be truncated
  and fallback engines will not work)
...
Make sure each CLI is authenticated for the same Linux user that will run the
bot.
...
- `VAULT`: optional path to a workspace/vault directory for file-saving agent
  skills (see Recommended Setup). Leave empty if you don't use one.
...
The default mode. The bot keeps a long-lived headless agent process per topic
and talks to it over the CLI streaming protocol. You get live progress,
message-by-message streaming (`live+`), the ability to send follow-up messages
into a running turn, and session resume — without a terminal UI.

Use this when:

- you mostly send tasks and read answers from Telegram;
- you want new messages injected into the agent's current turn;
- you do not need to see or steer the raw terminal.
...
- `live+` (default): live progress plus the agent's intermediate text replies
  streamed into the chat as separate 💬 blocks. The closest thing to watching
  the agent think and talk in real time.
- `live`: keeps one editable progress message and then sends the final answer.
  A good quieter option for most project work.
...
Slash commands are special in tmux topics: non-bot commands not owned by the bot, such as
...
- `/mode`: forum topics only; choose `streaming`, `tmux`, or `subprocess`.
  Switching away from `tmux` stops the active tmux session.
- `/engine`: forum topics only; choose Claude Code or Codex. Changing engine
  resets the active session.
- `/stream`: forum topics only; choose `live+`, `live`, `verbose`, or
  `minimal`.
- `/model`: pick the provider model for the current topic.
- `/health`: run read-only self-checks (CLI present and onboarded, writable
  paths, config) and report the results. The first thing to try when the bot
  misbehaves.
- `/sysprompt`: set a custom system prompt for the current context (forum topic
  or DM). The text is appended on top of the base persona, not replacing it.
  `/sysprompt <text>` sets it (multi-line or as a reply to a message);
  `/sysprompt` with no argument shows the current one; `/sysprompt reset` clears
  it. A topic stores it in its `system_prompt`; a DM stores it under
  `chat_prompts` keyed by `chat_id`. The prompt applies on a new session (CC
  restores the original prompt on `--resume`), so the bot offers an "apply now"
...
- Out of the box a new topic runs `streaming` execution with `live+` stream
  mode — keep these unless you have a reason not to.
- Real terminal-driven development (permission dialogs, TUI): `/mode` ->
  `tmux`.
- Short one-off tasks in a quiet chat: `/stream` -> `minimal`.
...
Based on [telegram-ai-agent](https://github.com/pavel-molyanov/telegram-ai-agent)
by Pasha Molyanov ([@molyanov_blog](https://t.me/+zJ5qmSsoYediYzdi)).
Developed and maintained by Aleksei (meriler).
