...
_TMP_FILE_MAX_AGE_SECONDS = 3 * 24 * 3600  # 3 days — для edit-итераций через ночь


def cleanup_old_tmp_files(file_cache_dir: str) -> int:
    """Delete stale files from the cache dir, including subdirectories.

    Recursive on purpose: runtime artifacts accumulate in subdirs too
    (mcp-runtime/ MCP configs, codex-last-message/) and used to leak
    forever because this cleanup only looked at the top level.
...
def _make_media_callback(
    ...
    async def on_media_batch(raw_messages: list[Message]) -> None:
        ...
        # engine now switches instead of bailing. photo/forward resolve the reply
        # target from the batcher's captured text-reply. (M4/M5, audit 2026-07-02.)
        reply_target = text_reply if text_reply else last_msg.reply_to_message
