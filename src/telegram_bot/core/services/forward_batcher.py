...
async def _download_video_note(
    message: Message,
    bot: Bot,
    tmp_dir: Path,
) -> Path | None:
    """Download a video_note (round video) mp4 to tmp dir."""
    try:
        if not message.video_note:
            return None
        vn = message.video_note
        if is_file_too_large(vn.file_size):
            logger.warning("Video note too large: %s", vn.file_size)
            return None
        timestamp = int(time.time())
        dest_path = tmp_dir / f"{timestamp}_{vn.file_unique_id}_videonote.mp4"
        await bot.download(vn.file_id, destination=dest_path)
        return dest_path
    except Exception:
        logger.warning(
            "Failed to download video_note (message_id=%s)",
            message.message_id,
            exc_info=True,
        )
        return None
...
async def _kill_if_alive(proc: asyncio.subprocess.Process | None) -> None:
    """Kill a subprocess that outlived its `asyncio.wait_for` timeout.

    `asyncio.wait_for` only cancels the *await*, not the OS process — a hung
    ffmpeg/ffprobe keeps running (up to 4 orphans per stuck video note). Call
    this in a `finally` so the process is reaped on timeout AND on any other
    error path. No-op if the process already exited. `transcriber.py` does the
    same with `proc.kill()`; this centralises it. (M2, audit 2026-07-02.)
    """
...
async def _extract_video_frames(video_path: Path, tmp_dir: Path) -> list[Path]:
    """Extract 3 JPEG frames (start, middle, end) from a video via ffmpeg.

    Returns list of paths (0-3 items) - empty if ffprobe/ffmpeg fails. Frames
    are passed to CC as photo attachments so the agent can "see" the kruzhochek.
    """
    # Probe duration via ffprobe (fast, no decode)
    ...
    if duration <= 0.0:
        ...
        timestamps = [0.1, duration / 2, max(0.1, duration - 0.2)]

    base = video_path.stem
    frame_paths: list[Path] = []
    for idx, ts in enumerate(timestamps):
        dest = tmp_dir / f"{base}_frame{idx}.jpg"
        ...
        try:
            ...
            logger.warning("ffmpeg frame extract failed at ts=%.3f", ts, exc_info=True)
        finally:
            await _kill_if_alive(proc)
    return frame_paths


async def _extract_audio_wav(video_path: Path, tmp_dir: Path) -> bytes | None:
    """Extract mono 16k WAV audio bytes from a video via ffmpeg. Returns None on failure."""
    dest = tmp_dir / f"{video_path.stem}_audio.wav"
    proc: asyncio.subprocess.Process | None = None
    try:
        proc = await asyncio.create_subprocess_exec(
            "ffmpeg",
        ...
        logger.warning("ffmpeg audio extract failed for %s", video_path, exc_info=True)
        return None
    finally:
        await _kill_if_alive(proc)
...
async def _process_video_note(
    ...
    """Download video_note, extract 3 frames + audio, transcribe.

    Returns (label_text, frame_paths). label_text is a CC-prompt-ready string
    like "[Video message, transcription]: hello" or a localized failure label.
    """
    label = t("cc.videomessage_label")
    fail = t("cc.transcription_failed", label=label)

    video_path = await _download_video_note(message, bot, tmp_dir)
    if video_path is None:
        return fail, []

    # Run frame extraction + audio extraction in parallel
    frames_task = asyncio.create_task(_extract_video_frames(video_path, tmp_dir))
    audio_task = asyncio.create_task(_extract_audio_wav(video_path, tmp_dir))
    ...
    if audio_bytes:
        for attempt in range(2):
            try:
                transcript = (await transcriber.transcribe(audio_bytes)).strip()
                break
            except TranscriptionError:
                if attempt == 0:
                    logger.info("video_note transcription failed, retrying")
                    continue
                logger.warning("video_note transcription failed after retry", exc_info=True)

    if transcript:
        text = f"[{label}, transcription]: {transcript}"
    else:
        text = f"[{label}, no speech detected]" if audio_bytes else fail
    return text, frames
...
async def _process_forwarded_message(
    ...
    # Video note (round) — transcribe audio + extract 3 frames so CC can "see" it.
    if message.video_note:
        vn_text, vn_frames = await _process_video_note(message, bot, transcriber, tmp_dir)
        parts.append(vn_text)
        for fp in vn_frames:
...
def _sanitize_sender_field(value: str) -> str:
    """Neutralize a forwarded sender's display field for safe prompt embedding.

    Display names/usernames come from the forwarded message origin and are
    fully attacker-controlled. They're placed on their own prompt lines
    (cc.forward_from), OUTSIDE the randomized delimiter that guards the message
    body — so a crafted name with newlines could forge extra prompt structure.
    Collapsing all whitespace (incl. newlines) to single spaces kills the
    newline-injection vector; the length cap bounds bloat. (S5, audit
    2026-07-02.)
    """
...
def _format_sender(sender: SenderInfo) -> str:
    """Format sender info for the prompt (sanitized — attacker-controlled)."""
    name = _sanitize_sender_field(sender.name)
    if sender.username:
        return f"{name} (@{_sanitize_sender_field(sender.username)})"
    return name
...
class ForwardBatcher:
        ...
        try:
            ...
            if buf_len >= self._max_batch_size:
                ...
                if channel_key in self._batches:
                    # Chain after any in-flight processing_task, exactly like
                    # _debounce does. Without this the force-flush started a
                    # SECOND parallel _process_batch on the same channel while
                    # the first was still running its slow text_cb/start_session
                    # — the same race _debounce was added to fix (prod incident
                    # 2026-04-27, dropped messages). (M1, audit 2026-07-02.)
                    prev = self._batches[channel_key].processing_task
                    self._batches[channel_key].timer = None
                    self._batches[channel_key].processing_task = asyncio.create_task(
                        self._run_processing_after(prev, channel_key, collected),
                    )
                return
    ...
    async def _run_processing_after(
        self,
        previous: asyncio.Task[None] | None,
        channel_key: ChannelKey,
        collected: list[Message],
    ) -> None:
        """Force-flush counterpart of _debounce's serialization: wait for the
        in-flight processing_task (if any) before starting this one, so a
        buffer-overflow flush can't run a parallel _process_batch on the same
        channel. (M1, audit 2026-07-02.)"""
        if previous is not None and not previous.done():
            logger.info("Force-flush for %s waiting on in-flight processing_task", channel_key)
            with contextlib.suppress(Exception):
                await previous
        await self._run_processing(channel_key, collected)
    ...
    async def _process_batch(self, channel_key: ChannelKey, batch: list[Message]) -> None:
        ...
        elif voice_snapshot and not self._transcriber:
            logger.warning(
                "Voice messages received but transcriber is None — skipping transcription"
            )
            for _, recog_msg in voice_snapshot:
                await _edit_recog_msg(recog_msg, t("cc.voice_failed_full"))
        ...
        media_snapshot_size = len(cb.media_buffer)
        ...
        del cb.comment[:comment_snapshot_size]
        del cb.message_ids[:msg_ids_snapshot_size]
        # Partial clear (snapshot prefix only), mirroring `cb.comment` above.
        # A blanket `cb.media_buffer.clear()` would drop photos/documents that
        # arrived via `add_media` DURING the (possibly slow) media_cb await —
        # the next debounce would then find an empty buffer and log
        # "no_callback_matched ... message lost!". This is the media twin of
        # the comment/voice fixes. (C1, audit 2026-07-02.)
        del cb.media_buffer[:media_snapshot_size]
        cb.voice_snapshot = []
