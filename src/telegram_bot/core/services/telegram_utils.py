    ...
    *,
    ...
    try:
        ...
        try:
            sent = await send_html()
            path = "html_retry"
        except TelegramForbiddenError as fe:
            # The bot can get blocked DURING the retry, not just on the first
            # attempt — propagate fatal so callers stop sending further chunks
            # to this chat. (M7, audit 2026-07-02.)
            logger.warning("Bot blocked on %s (retry): %s", label, fe)
        ...
        try:
            ...
            # Blocked mid-fallback → fatal, same reasoning as the retry branch. (M7)
            logger.warning("Bot blocked on %s (plain fallback): %s", label, fe)
            return SendOutcome(message_id=None, fatal=True)
        except Exception:
