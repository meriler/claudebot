...
def _standard_bot_server(project_root: Path) -> dict[str, Any]:
    # Resolve to absolute path: CC subprocess launches MCP servers from the
    # topic's cwd (e.g. ~/vault), not from project_root. A relative path here
    # makes bash silently fail to find the script, leaving the agent without
    # the `send_document`/`send_image`/`send_message` tools.
    root_abs = project_root.resolve()
    return {
        "command": "bash",
        "args": [str(root_abs / "mcp-servers" / "bot" / "start.sh")],
        "env": {"PROJECT_DIR": str(root_abs)},
    }
...
def ensure_bot_runtime_mcp_config(
    *,
    ...
    # Ensure args/PROJECT_DIR are absolute — CC subprocess launches MCP from
    # the topic's cwd, not from project_root. Relative paths silently break
    # the bot MCP server.
    if "args" in bot_server and isinstance(bot_server["args"], list):
        bot_server["args"] = [
            str((root / a).resolve()) if not os.path.isabs(a) and "start.sh" in a else a
            for a in bot_server["args"]
        ]
    raw_env = bot_server.get("env")
    env = dict(raw_env) if isinstance(raw_env, dict) else {}
    env["PROJECT_DIR"] = str(root)  # always overwrite — must be absolute
