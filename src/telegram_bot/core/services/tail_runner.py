        ...
        try:
            while True:
                ...
                if now - last_alive_check >= ALIVE_CHECK_INTERVAL:
                    ...
                    # Offload the blocking `tmux has-session` to a thread — this
                    # runs every ALIVE_CHECK_INTERVAL per live tail, so a sync
                    # subprocess.run here stalls the loop under multiple sessions.
                    # (H6, audit 2026-07-02.)
                    if not await asyncio.to_thread(self._tmux_alive, session_name):
