...
def _tool_status_map() -> dict[str, str]:
    ...
    mapping: dict[str, str] = {
        ...
        "mcp__bot__send_url_image": t("tool.send_image"),
    }
...
class StreamEvent:
    ...
    type: Literal[
        "status", "text", "thinking", "result", "result_message", "media_url", "usage_data"
    ]
    content: str
    ...
    if event_type == "system":
        ...
        for block in (data.get("message") or {}).get("content", []):
            ...
            if block_type == "tool_use":
                ...
                # Reasoning block. Emitted unconditionally; the renderer decides
                # whether to surface it (only stream_mode == "live+" with the
                # stream_thinking toggle on). Every other mode ignores it.
                thinking = block.get("thinking", "")
                if thinking:
                    events.append(StreamEvent("thinking", thinking))

    elif event_type == "rate_limit_event":
        rl_info = data.get("rate_limit_info") or {}
        events.append(
            StreamEvent(
                "usage_data",
                "",
                session_id=data.get("session_id"),
                usage_info={"rate_limit": rl_info},
            )
        )
        ...
        if model_usage or total_cost is not None:
            events.append(
                StreamEvent(
                    "usage_data",
                    "",
        ...
        for block in (data.get("message") or {}).get("content", []):
            if block.get("type") == "tool_result":
                _extract_media_urls(block, events)

    return events, session_id


def _extract_media_urls(block: dict[str, Any], events: list[StreamEvent]) -> None:
    ...
    if isinstance(url, str) and url.startswith("https://"):
        events.append(StreamEvent("media_url", url))
