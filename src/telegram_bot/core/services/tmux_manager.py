...
# Background checkpoint-on-reset reaper tuning (see `_reap_parked_session`).
# A parked TUI is polled until its pane stops changing for `IDLE_STABLE` —
# the elapsed-time counter / spinner ticks only while the engine works, so a
# stable pane means the checkpoint is done. `GRACE` lets the prompt actually
# start rendering before the first poll; `MAX_RUNTIME` is a hard backstop so a
# wedged checkpoint can never leak a tmux session forever.
_CHECKPOINT_GRACE_SEC = 15.0
_CHECKPOINT_POLL_SEC = 4.0
_CHECKPOINT_IDLE_STABLE_SEC = 12.0
_CHECKPOINT_MAX_RUNTIME_SEC = 600.0
...
class TmuxManager:
    ...
    def __init__(
        ...
        *,
        ...
        # Usage tracker plumbing, wired via wire_usage_tracker().
        self._usage_tracker: object | None = None
        self._usage_settings: object | None = None
        ...
        # Background reaper tasks for parked checkpoint sessions (see
        # `clear_context` / `_park_and_checkpoint`). Held to keep a strong
        # reference so the GC cannot drop a running task mid-checkpoint.
        self._checkpoint_reapers: set[asyncio.Task[None]] = set()
    ...
    def wire_usage_tracker(self, *, usage_tracker: object, settings: object) -> None:
        """Attach UsageTracker for pinned usage messages."""
    ...
    async def start_session(
        ...
        *,
        ...
        await self._maybe_start_usage_pin(channel_key, chat_id, name)
    ...
    async def _spawn_tmux(
        ...
        *,
        ...
        new_session_argv = [
            ...
            "-y",
            "50",
            # Pin CC's auto-compact window to the model's full 1M context. Without
            # this a CC rollout buckets these headless sessions to a reduced ~700k
            # window, so auto-compact fires at ~668k instead of near 1M (an
            # interactive terminal on the same account/model reaches ~1M). CC caps
            # the value at the model's real window (min(realWindow, N)), so it can
            # never exceed capacity — safe for 200k models like Haiku too, and a
            # no-op for the codex provider. `-e` sets it per-pane, bypassing the
            # tmux server's cached environment.
            "-e",
    ...
    async def _check_transcript_tail_health(
        ...
        *,
        ...
        if state is None or not state.session_id:
            ...
            return
        if not await self._tmux_alive_async(state.session_name):
            self._clear_transcript_lag(channel_key)
            return
    ...
    async def cancel(self, channel_key: ChannelKey) -> None:
        ...
        if state:
            ...
            try:
                ...
                logger.warning("TUI_IO: cancel send-keys timed out for %s", state.session_name)
    ...
    async def clear_context(
        ...
        *,
        ...
        `checkpoint_prompt`: when set, the live TUI is *parked* under a side
        tmux session and handed this prompt to write a background checkpoint
        before the fresh session takes its place. The parked session is not
        tailed (silent — no Telegram output) and is reaped once it goes idle.
        Parking is best-effort: a rename/send failure falls back to the normal
        respawn, which kills the old pane as before.

        Returns True if a live tmux session was found and respawned,
        False if tmux was already dead.
        """
        state = self._sessions.get(channel_key)
        if not state or not await self._tmux_alive_async(state.session_name):
            return False
        ...
        # Park the live TUI for a background checkpoint before it is replaced.
        # Must run after the tail is fully detached (above) so the parked
        # session is never tailed, and before the respawn below so the freed
        # session name is available for the fresh `new-session`.
        if checkpoint_prompt:
            parked = await self._park_and_checkpoint(
            ...
            if parked is not None:
                logger.info(
                    "TUI_IO: parked session=%s as %s for background checkpoint",
                    state.session_name,
                    parked,
                )
        ...
        await self._maybe_start_usage_pin(channel_key, state.chat_id, state.session_name)
        return True

    async def _park_and_checkpoint(
        self,
        *,
        old_name: str,
        prompt: str,
        ...
        """Rename the live tmux session aside and ask it to checkpoint.

        Renames `old_name` to `<old_name>-ckpt-<hex8>` so the original name is
        freed for the caller's fresh `new-session`, then interrupts any
        in-flight work and pastes `prompt` into the parked TUI. A reaper task
        kills the parked session once it idles. Returns the parked session
        name, or None if parking could not be set up (caller then proceeds
        with the normal respawn, which kills the old pane).
        """
        _ = provider  # generic across claude/codex; kept for call-site clarity
        parked = f"{old_name}-ckpt-{generate_session_uuid().split('-')[0]}"
        rename = await asyncio.to_thread(
            subprocess.run,
            ["tmux", "rename-session", "-t", old_name, parked],
            capture_output=True,
            text=True,
        )
        if rename.returncode != 0:
            logger.warning(
                "checkpoint park: rename %s -> %s failed (%s); skipping checkpoint",
                old_name,
                parked,
                rename.stderr.strip(),
            )
        ...
        # Interrupt any in-flight work so the input bar accepts the prompt.
        with contextlib.suppress(Exception):
            await asyncio.to_thread(
                subprocess.run,
                ["tmux", "send-keys", "-t", parked, "Escape"],
                capture_output=True,
                timeout=5,
            )
        ...
        try:
            await send_text_to_tmux(parked, prompt, submit_enter=True)
        except Exception:
            logger.warning(
                "checkpoint park: failed to send prompt to %s; reaping", parked, exc_info=True
            )
            await asyncio.to_thread(
                subprocess.run,
                ["tmux", "kill-session", "-t", parked],
                capture_output=True,
            )
            return None

        task: asyncio.Task[None] = asyncio.create_task(self._reap_parked_session(parked))
        self._checkpoint_reapers.add(task)
        task.add_done_callback(self._checkpoint_reapers.discard)
        return parked

    async def _reap_parked_session(self, name: str) -> None:
        """Kill a parked checkpoint session once it goes idle or times out.

        Polls the pane: the engine's elapsed-time counter / spinner advances
        only while it works, so a pane unchanged for `_CHECKPOINT_IDLE_STABLE_SEC`
        means the checkpoint has finished. `_CHECKPOINT_MAX_RUNTIME_SEC` is a
        hard backstop against a wedged session leaking forever.
        """
        deadline = time.monotonic() + _CHECKPOINT_MAX_RUNTIME_SEC
        last_pane: str | None = None
        stable_since: float | None = None
        await asyncio.sleep(_CHECKPOINT_GRACE_SEC)
        try:
            while time.monotonic() < deadline:
                if not await self._tmux_alive_async(name):
                    logger.info("checkpoint session %s exited on its own", name)
                    return
                try:
                    pane: str | None = await capture_pane(name)
                except (OSError, subprocess.SubprocessError):
                    pane = None
                if pane is not None:
                    if pane == last_pane:
                        ...
                        elif time.monotonic() - stable_since >= _CHECKPOINT_IDLE_STABLE_SEC:
                            logger.info(
                                "checkpoint session %s idle %.0fs -> reaping",
                                name,
                                _CHECKPOINT_IDLE_STABLE_SEC,
                            )
                            break
                    else:
                        last_pane = pane
                        stable_since = None
                await asyncio.sleep(_CHECKPOINT_POLL_SEC)
            else:
                logger.warning(
                    "checkpoint session %s hit max runtime %.0fs -> reaping",
                    name,
                    _CHECKPOINT_MAX_RUNTIME_SEC,
                )
        finally:
            await asyncio.to_thread(
                subprocess.run,
            ...
            logger.info("Reaped checkpoint session %s", name)

    async def reap_orphan_checkpoint_sessions(self) -> int:
        """Kill any leftover `*-ckpt-*` sessions from a previous bot run.

        Parked sessions are intentionally not persisted, so a restart mid-
        checkpoint would orphan them with no reaper. Called once on startup.
        ...
        for line in result.stdout.splitlines():
            ...
            killed += 1
            logger.info("Reaped orphan checkpoint session %s on startup", name)
        return killed
    ...
    async def switch_or_start_session(
        ...
        *,
        ...
        async with self._get_channel_lock(channel_key):
            ...
            if (
                captured
                and await self._tmux_alive_async(captured.session_name)
            ...
            if captured and await self._tmux_alive_async(captured.session_name):
                await self.close_buffer(channel_key)
    ...
    async def _maybe_start_usage_pin(
        self, channel_key: ChannelKey, chat_id: int, tmux_session_name: str
    ) -> None:
        tracker = self._usage_tracker
        if tracker is None:
            return
        settings = self._usage_settings
        if settings is not None and not getattr(settings, "usage_pin_enabled", True):
            return
    ...
    async def _tmux_alive_async(self, session_name: str) -> bool:
        """Async `tmux has-session` — offloads the blocking subprocess.run to a
        thread so the event loop is not stalled. Use this from coroutines;
        `_tmux_alive` stays sync for startup/non-async paths. Wraps the instance
        `self._tmux_alive` (not the module fn) so test patches of `_tmux_alive`
        apply here too. (H6, audit 2026-07-02.)"""
        return await asyncio.to_thread(self._tmux_alive, session_name)
