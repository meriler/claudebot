...
# Header for continuation pages. A function (not a constant) so a runtime
# /language switch is picked up instead of freezing the import-time value.
def _header_continued() -> str:
    return t("ui.thinking") + "\n"


# Moscow is a fixed UTC+3 offset (no DST since 2014), so a static timezone is
# simpler and more robust than depending on the tzdata "Europe/Moscow" entry.
_MSK = timezone(timedelta(hours=3))


def _ts() -> str:
    """HH:MM:SS prefix in Moscow time (the operator's timezone)."""
...
class LiveStatusBuffer:
    ...
    def has_content(self) -> bool:
        """True if any status line was ever recorded (current or rotated page).

        Used by the empty-turn cleanup: a buffer that never got a line leaves
        the initial placeholder message showing bare "Thinking..." — safe to
        delete; one with history must be kept.
        """
        return bool(self._lines) or len(self._message_ids) > 1
